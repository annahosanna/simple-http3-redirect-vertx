...
public class HttpVertical extends VerticleBase {
  ...
  public Future<?> start() throws Exception {
    ...
    SocketAddress bindSocketAddress = SocketAddress.inetSocketAddress(8080,"127.0.0.1");
//    server = 
//    server.
    return vertx.createHttpServer().requestHandler(router).listen(bindSocketAddress);
  }
  ...
  private byte[] ip4AddressToByteArray(String dottedDecimal) {
	  ...
	  byte[] returnBytes = {(byte)Integer.parseInt(parts[0]),(byte)Integer.parseInt(parts[1]),(byte)Integer.parseInt(parts[2]),(byte)Integer.parseInt(parts[3])};
	  return returnBytes;
	  }
	  } catch (Exception e) {
	  byte[] returnBytes = {0,0,0,0};
